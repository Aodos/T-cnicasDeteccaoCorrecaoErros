...
public class Hamming {
	...
	private void simulandoErro() {
		...
		case "1":
			...
			//Alteração do Thiago
			char inverso;
			inverso= mensagemErradaMaisRedundantes.get(posicaoErrada-1);
			if(inverso=='1') {
				inverso='0';
			}else {
				inverso='1';
			...
			mensagemErradaMaisRedundantes.set(posicaoErrada-1, inverso);
			String msg2;
			msg2="";
			for(int i = mensagemErradaMaisRedundantes.size()-1; i >= 0 ; i--) {
				msg2 = msg2.concat(mensagemErradaMaisRedundantes.get(i).toString());	
			}
			JOptionPane.showMessageDialog(null, "Mensagem \"Corrigida\" = "+msg2, "Resultado", JOptionPane.INFORMATION_MESSAGE);
			//end bagunça do Thiago
			break;
		case "2":
			break;
		}
